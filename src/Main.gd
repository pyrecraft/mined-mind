...
func _on_HashInterface_alert_pressed():
	if len(GameState.hash_history) > GameState.hash_history_size_last_alert:
		GameState.hash_history_size_last_alert = len(GameState.hash_history)
...
func _on_HashInterface_sent_request(hash_val):
	...
	if str(hash_val) != "":
		GameState.hash_history.push_front(hash_val)
		hash_interface_child.update_response_field(GameState.build_hash_json_response())
		if GameState.alert_count == 0 && len(GameState.hash_history) == 1:
			$TextBox.queue_hash_code_sent_first_request()
		
	if GameState.requests_since_alert_updated == 3:
		match GameState.alert_count:
			1:
...
#				$TextBox.queue_single_line_grey("..some still thought it was part of the game after playing for minutes, hours.. even days..")
			...
			_:
				pass
	elif GameState.requests_since_alert_updated == 4:
		match GameState.alert_count:
			1:
				pass
#				$TextBox.queue_single_line_grey("..stanford prison experiment. They kept going because they were told to do something and were afraid of the repercussions..")
			2:
				pass
#				$TextBox.queue_single_line_grey("..some enjoyed the \"meta\" aspect of the game such as the self-referential signals, while others criticized it as \"corny\" or \"cheap\"")
			...
			_:
				pass
	elif GameState.requests_since_alert_updated == 9:
		match GameState.alert_count:
			1:
				pass
...
func _on_NumberGuess_sent_guess(guess_num):
	if GameState.current_state != GameState.state.NUMBER_GUESS_WAITING:
		print("Currently already processing a guess.")
		return
	print("Guessed " + str(guess_num))
	GameState.guess_nums.push_front(guess_num)
	if is_first_guess and guess_num == GameState.random_number:
		print("Number " + str(guess_num) + " was guessed right on the first try")
		randomize()
		if guess_num == 1:
		...
		print("New guess num is " + str(GameState.random_number))
		...
		print("Number guessed incorrectly")
		GameState.set_state(GameState.state.NUMBER_GUESS_INCORRECT)
...
func _on_GameOverDelayTimer_timeout():
	print("Going to " + str(queued_next_state))
	GameState.set_state(queued_next_state)
